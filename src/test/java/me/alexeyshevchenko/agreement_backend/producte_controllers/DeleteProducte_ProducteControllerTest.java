...
public class DeleteProducte_ProducteControllerTest {
    ...
    public void deleteUserById() throws Exception {
        ...
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders.delete("/products/"+producte.getId());
        Mockito.when(productService.getProductById(Mockito.anyLong())).thenReturn(savedProducte);
        mockMvc.perform(request)
                .andExpect(MockMvcResultMatchers.status().isOk())
                .andExpect(MockMvcResultMatchers.content().contentType(contentType))
                .andExpect(MockMvcResultMatchers.jsonPath("$.name", Matchers.is(savedProducte.getName())));
